...
// The events endpoint's Type parameter is a 2x2 selector, not an event kind: one axis is
// part-day vs full-day, the other is "starts inside the window" vs "started earlier and is
// still ongoing". The API rejects anything else with "The Type only allows '0', '1', '2', or
// '3'". A window query therefore has to ask for all four quadrants and merge them — asking
// only for Type=0 (the previous behaviour) silently dropped every full-day event, and every
// event that began before the window but reaches into it.
...
// collectRawEvents queries every Type quadrant for the window and returns the union, still
// encrypted and de-duplicated by event ID.
...
// eventsPage walks every page of one Type quadrant and returns the still-encrypted events.
